        ...
        if args.dataset_name is not None:
            ...
            for path in list(Path(instance_data_root).iterdir()):
                if path.suffix.lower() in valid_exts:
                    ...
                    # load per-image caption from .txt file
                    txt_path = path.with_suffix('.txt')
                    if txt_path.exists():
                        caption_map[path.name] = txt_path.read_text(encoding='utf-8').strip()
            self.custom_instance_prompts = list(caption_map.values()) if caption_map else None
